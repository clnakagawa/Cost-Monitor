...
def main():
    ...
    summTblCharr = pd.concat([load_char(sid,path) for sid,path in zip(data['name'], data['attributes.charr_output']) 
                             if isinstance(path, str) and "contamination" in path])
    summTblCharr.to_csv(sys.argv[1].replace("_attributes.tsv", "_charr.tsv"), sep='\t',
                   index = False)

    summTblDemo = pd.concat([load_demo(sid,path) for sid,path in zip(data['name'], data['attributes.demographics_file'])
